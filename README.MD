...
-  Внешние ключи -  <i>singularized_table_name_id</i> (т.е., <i>item_id</i>, <i>order_id</i>)
...
точно так же можно и удалять филды
...
список доступных в миграци комманд (некоторые из этих команд доступны только в Rails 4):

    create_table
    create_join_table
    drop_table
    drop_join_table
    change_table (удаление филдов, переименование, создание новых, индексы)
    rename_table
...
Валидации используются, чтобы быть уверенными, что только валидные данные сохраняются в вашу базу данных
...
<b>привязка форм к обектам ActiveRecord</b><br/>
...
params также содержит <i>controller_name</i>, <i>action_name</i>, и другие параметры из роутинга, например:
    ...
    # /clients/active will hit clients#index with params = {status:'active', foo:'bar'}
...
фильтры можно описывать как в виде фугкций, так и в виде классов
...
<i>app/mailers/user_mailer.rb<i>
...
<i>app/views/user_mailer/welcome_email.html.erb<i>
...
*  кэширвоание страницы
*  кэширование экшена
*  кэширование фрагмента


кэширование страницы (in <i>rails 4</i> removed from core to gem <i>'actionpack-page_caching'</i>)
...
кэширование экшена (in <i>rails 4</i> removed from core to gem <i>'actionpack-action_caching'</i>)
...
(in <i>rails 4</i> removed from core to gem <i>'rails-observers'</i>)
...
Файлопровод представляет фреймворк для соединения и минимизации <i>JavaScript</i> и <i>CSS</i>. 
...
Поддерживаемые языки по умолчанию: <i>Sass</i> для <i>CSS</i>, <i>CoffeeScript</i> для <i>JS</i> и <i>ERB</i> для обоих.
...
чтобы ощутить приимущества диджест меток файлов, необходимо настроить вэб-сервер на вечное 
...
<b>UNIT-тесты для моделей</b>


Перед началом тестирования нужно загнать данные в БД.
Эти данные хранятся в фикстурах. Фикстуры хранятся в <i>test/fixtures</i> и могут быть описаны в YAML или кодом в ERB.
